...
public class SmithingScreen extends SkillScreen {
    ...
    public static void setLevel(Integer level) {
        SmithingScreen.level = level;
    }
    
    @Override
    protected MutableText skillDescription() {
        return Text.translatable("skills.descriptions.rpmechanics.smithing");
    }
    ...
    protected ItemStack skillIcon() {
        return Items.ANVIL.getDefaultStack();
    }
    ...
    protected List<SkillStatistic> information() {
        ...
        stats.add(new SkillStatistic(
                Text.translatable("skills.rpmechanics.smithing.material_save"),
                Text.literal("+" + (skillLevel()-1) + "%")
        ));
        stats.add(new SkillStatistic(
                Text.translatable("skills.rpmechanics.smithing.exp_save"),
                Text.literal("+" + (skillLevel()-1) + "%")
        ));
        return stats;
    }
    ...
    protected Float baseExpReq() {
        return RoleplayMechanicsCommon.OWO_CONFIG.experienceOptions.smithingOptions.baseExperience();
    }
    
    @Override
    protected Enum<QuesConfigModel.Choices> increaseMethod() {
        return RoleplayMechanicsCommon.OWO_CONFIG.experienceOptions.smithingOptions.multiplicativeOrAdditive();
    }
    
    @Override
    protected Double amountToIncrease() {
        return RoleplayMechanicsCommon.OWO_CONFIG.experienceOptions.smithingOptions.amount();
    }
